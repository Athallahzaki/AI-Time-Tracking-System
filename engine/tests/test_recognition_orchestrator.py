...
from engine.plugins.face_recognizer.contracts.identity import RecognitionState
...
class FakeRecognitionPolicy:
    ...
    def should_recognize(self, track, state, current_time):
        self.called = True
...
def test_orchestrator_skips_when_policy_denies_recognition():
    ...
    cropper = FakePersonCropper(
        result=np.ones((80, 40, 3), dtype=np.uint8),
    )

    preprocessor = FakeImagePreprocessor(
        result=np.ones((112, 112, 3), dtype=np.uint8),
    )
